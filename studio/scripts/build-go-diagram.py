...
def node(x, y, icon, label, sub=None, accent=GO, r=30, size=None):
    ...
    p.append(f'<text x="{x}" y="{y+r+24}" font-size="18" font-weight="700" fill="{INK}" text-anchor="middle">{esc(label)}</text>')
    for i, line in enumerate(sub or []):
        p.append(f'<text x="{x}" y="{y+r+44+i*18}" font-size="15" fill="{FAINT}" text-anchor="middle">{esc(line)}</text>')
...
def packets(d, accent, count=3, dur=2.8, size=9, kind="frame"):
    ...
    A dashed stroke says "something flows here". A frame that you can watch
    enter one node and arrive at the next says what is actually moving, which
    for this control plane is the whole point: these are admitted events, not a
    pipe of anonymous bytes.
    """
    half = size / 2
    for i in range(count):
        begin = f"{(dur / count) * i:.2f}s"
        if kind == "frame":
            body = (f'<rect x="{-half:.1f}" y="{-half:.1f}" width="{size}" height="{size}" rx="2" '
                    f'fill="{accent}" stroke="{PANEL}" stroke-width="1.5"/>')
        else:
            body = f'<circle r="{half:.1f}" fill="{accent}"/>'
...
def pulse(x, y, r, accent, dur=2.4):
    """A ring leaving a node: this one is doing work right now."""
...
def note(x, y, t, c=DIM, size=15, anchor="start", weight="400"):
...
def region(x, y, w, h, label, sub, accent):
    ...
    if sub: p.append(f'<text x="{x+26}" y="{y+56}" font-size="15" fill="{FAINT}">{esc(sub)}</text>')

W, H = 2400, 1080
p.append(f'<g transform="translate(44 20) scale(1.15)">{GOPHER}</g>')
p.append(f'<text x="132" y="62" font-size="34" font-weight="700" fill="{INK}" letter-spacing="2">GO CONTROL PLANE</text>')
note(560, 62, "studio-backend/ · deterministic · zero LLM", FAINT, 18)
note(2356, 62, "the browser reads it but never owns it", GO, 15, "end")
...
pulse(760, 320, 34, GREEN, 2.2)
note(760, 440, "a frame is data, never a command", RED, 15, "middle")
for i in range(3):
    lane = curve(228, 250 + i * 152, 726, 320, 0.06, GO, head=False)
...
for i, st in enumerate(STATES):
    ...
    p.append(f'<text x="{x+ox:.0f}" y="{y+oy:.0f}" font-size="15" font-weight="{"700" if gate or done else "400"}" '
...
note(cx, cy + 16, "one human decision", GOLD, 15, "middle", "700")
note(cx, cy + 36, "bound to an exact digest", FAINT, 14, "middle")
packets(curve(794, 320, 1250, 340, 0.05, GREEN), GREEN, count=3, dur=2.6, size=10)
...
note(2110, 664, "on disagreement Cloud SQL wins", RED, 14, "middle")
note(2110, 682, "and execution fails closed", RED, 14, "middle")

node(1200, 830, "activity", "SSE fan-out", ["text/event-stream · resumable by frame id"], GO, 30)
pulse(1200, 830, 30, GO, 1.8)
...
<style>
  ...
  @media (prefers-reduced-motion: reduce) {{
    .dg-flow {{ animation: none; opacity: .75; }}
    /* animateMotion is not CSS and cannot be paused from here, so the moving
       parts are removed outright rather than left running unseen. */
