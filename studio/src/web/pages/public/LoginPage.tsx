...
export function LoginPage({
  ...
  const isLocalDemo = authMode === "local_demo";
    ...
    <div className="public-page">
      ...
      <main id="login-content" className="public-main login-page-container" tabIndex={-1}>
        <section className="login-chassis" aria-labelledby="login-title">
          <header className="login-chassis__header"><PixelIcon name={isAuthenticated ? "shield-check" : "lock"} size="lg" color={isAuthenticated ? "google-green" : "google-blue"} glow /><h1 id="login-title" className="login-chassis__title">{isAuthenticated ? "Authenticated session" : isLocalDemo ? "Enter local demo control" : "Sign in to migration control"}</h1><p className="login-chassis__tagline">{isLocalDemo ? "This identity exists only in the loopback demo process." : "Identity is configured by the hosting application."}</p></header>
          <div className="login-chassis__body">
            ...
            {isAuthenticated ? <>
              ...
              <button type="button" className="login-google-btn" onClick={onSignIn} disabled={!canSignIn} aria-describedby="token-flow"><PixelIcon name="key" size="sm" color="google-blue" /><span>{authStatus === "unconfigured" ? "Sign-in not configured" : authStatus === "initializing" ? "Preparing sign-in" : isLocalDemo ? "Enter as local demo operator" : "Sign in with Firebase"}</span></button>
              <div id="token-flow" className="login-security-notice"><strong>IDENTITY FLOW</strong><p>{isLocalDemo ? "The development server accepts one non-secret demo identity on loopback only. It cannot be enabled in a production build or used from another host." : "Firebase obtains an ID token. The browser sends that token to a same-origin endpoint, where the backend-for-frontend verifies it before creating application session state. The page does not expose provider secrets, cloud credentials, or authorization scopes."}</p></div>
