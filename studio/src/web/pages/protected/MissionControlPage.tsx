...
const CARTRIDGES: readonly Cartridge[] = [
  ...
  { id: 'dynamics', label: 'Microsoft Dynamics AX 2012 R3 / SQL Server', host: 'dynamics-ax',
    defect: 'one logical row split across two physical tables',
    driver: 'Microsoft SQL Server', driverNote: 'synthetic emulator speaks SQL directly',
    jar: 'mssql-jdbc.jar', jarNote: 'declared for the production SQL Server path; fingerprinted before use',
    issues: [
      { code: 'AX-CUSTTABLE-001', title: 'Orphan derived record',
        detail: 'CustTable extends DirPartyTable and carries only its own columns. A derived row whose base row is absent has no name, and is quarantined rather than landed half-formed.' },
      { code: 'AX-CUSTTABLE-002', title: 'Cross-partition identity binding',
        detail: 'RecId is unique only inside one company and one partition. A RecId that resolves under a different partition is a different entity, and binding the two is refused.' },
      { code: 'AX-SQLDICT-003', title: 'Physical names are not application names',
        detail: 'SQLDICTIONARY and MODELELEMENT hold the mapping from CUSTTABLE to CustTable and its inheritance. The warehouse schema is derived from that catalogue, never from the physical table name.' },
    ] },
  { id: 'ebs', label: 'Oracle E-Business Suite 19c / Oracle 19c', host: 'oracle-ebs-19c',
    defect: 'generic columns whose meaning lives in a metadata catalogue',
    driver: 'Oracle Database 19c', driverNote: 'synthetic emulator speaks SQL directly',
    jar: 'ojdbc8.jar', jarNote: 'declared for the production Oracle path; fingerprinted before use',
    issues: [
      { code: 'EBS-HZPARTIES-001', title: 'Descriptive flexfields',
        detail: 'ATTRIBUTE1 is customer_tier in one context and payment_profile in the next, in the same column of the same table. FND_DESCRIPTIVE_FLEXS resolves it by application, table, context, segment and metadata version.' },
      { code: 'EBS-HZPARTIES-002', title: 'Unmapped context',
        detail: 'A row whose attribute_category has no mapping at the declared metadata version cannot be named without guessing, so it is quarantined instead of landed untyped.' },
    ] },
]
